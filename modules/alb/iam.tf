resource "aws_iam_role" "eks_controller_role" {
  name = "${var.project_name}-aws-alb-controller"
    ...
    "Statement": [
      {
        ...
        "Condition": {
            "StringEquals": {
                "oidc.eks.${data.aws_region.current.name}.amazonaws.com/id/${local.oidc}:sub": "system:serviceaccount:kube-system:aws-alb-ingress-controller",
